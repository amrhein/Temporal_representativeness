...
fun =   @(tau0,taua,taux,tauy,b,d)...
        quadgk(@(x)funi(taua,taux,tauy,b,d,x),1/tau0,Inf);
...
% Denominator of the error function
fund =  @(tau0,taux,b)...
        quadgk(@(x)fundi(taux,b,x),1/tau0,Inf);
...
fun_a = @(tau0,taua,taud,taux,tauy,b,d)quadgk(@(dd)...
        1/((taud)*sqrt(2*pi)) * exp(-(d-dd).^2./(2*(taud+eps).^2)).* ...   % Gaussian PDF
        fun(tau0,taua,taux,tauy,b,dd),-Inf,Inf);
