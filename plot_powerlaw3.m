...
b = .5;
out5 = est_error_powerlaw(t0v,tav,tdv,txv,tyv,dv,b);

b = 1.5;
out15 = est_error_powerlaw(t0v,tav,tdv,txv,tyv,dv,b);
...
for ii = 1:length(outvec)
    ...
    clabel(C,h,'manual')
    export_fig('-png','-r200',['Figs/errors_wrt_beta_' char(outvec(ii)) '_tau0_' num2str(t0)])    
...
for ii = 1:length(outvec)
    ...
    plot([1000 1000],[0,4000],'k-','LineWidth',1.5)
    clabel(C,h,'manual')
    export_fig('-png','-r200',['Figs/errors_wrt_beta_taua_' char(outvec(ii)) '_tau0_' num2str(t0)])    
