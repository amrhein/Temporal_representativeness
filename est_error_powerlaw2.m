function out = est_error_powerlaw2(t0v,tav,tdv,txv,tyv,dv,b)
...
% slow:
%     'method','iterated','AbsTol',1e-7,'RelTol',1e-5...
% 'AbsTol',1e-8,'RelTol',1e-5...
% Nice and smooth:
%    'method','iterated','AbsTol',1e-8,'RelTol',1e-5...
% Relatively smooth...
%    'method','iterated','AbsTol',1e-7,'RelTol',1e-4...

%'RelTol',1e-4,'MaxFunEvals',600 ... % some errors

%fun_a = @(tau0,taua,taud,taux,tauy,b,d)integral(@(dd)...
%        1/((taud)*sqrt(2*pi)) * exp(-(d-dd).^2./(2*(taud+eps).^2)).* ...   % Gaussian PDF
%        fun(tau0,taua,taux,tauy,b,dd),-Inf,Inf,'Arrayvalued',true,'RelTol',1e-2);
